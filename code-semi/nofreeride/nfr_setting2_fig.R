# ==============================================================================
# SCRIPT: Main-text Setting (b), including the local barrier in Algorithms 1
#         and 2 (w = 100) for both OMDRC arms.
#
# DGP: F0 = N(0,1), F1 = N(3,1), with the Gaussian prevalence surge
# 0.05 -> 0.30 -> 0.05 specified in the main text.
...
# prevalence (low -> peak -> low, 0.05 -> 0.30 -> 0.05). The peak is placed just
# right of centre (surge_center = 0.60) so the low-pi tail after the peak is
# short; combined with the short window D this keeps OMDRC.DD's FDR at/below
# Static Lmdr's across the later segment. Under the surge, RTK
...
fig_dir <- nfr_dir
results_file <- file.path(fig_dir, "NFR_Setting2_results.rds")
reuse_cache <- identical(Sys.getenv("NFR_REUSE_CACHE"), "1")

cat(sprintf("Starting Setting 2 + NFR credited barrier (d=%d)...\n", D_NFR))
...
ggsave(file.path(fig_dir, "NFR_Setting2_fig.pdf"),
       plot = final_plot2, width = 10, height = 5)
ggsave(file.path(fig_dir, "NFR_Setting2_fig.png"),
       plot = final_plot2, width = 10, height = 5, dpi = 300)
cat("Figures written to", file.path(fig_dir, "NFR_Setting2_fig.pdf/png"), "\n")
