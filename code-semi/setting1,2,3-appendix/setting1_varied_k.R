# ==============================================================================
# APPENDIX SCRIPT: Online MDR Control - Setting 1 sensitivity to the Gamma shape k
#
# This is the APPENDIX companion of code-semi/setting1,2,3/setting1.R. It keeps
# the SAME methodology as the main Setting 1 script:
#   * skewed components F0 = Exp(1), F1 = Gamma(shape_alt = k, scale = 1);
#   * smooth DECREASING prior-drop prevalence path pi_t (high -> low);
...
#     confidence bands.
# The only change is that we SWEEP the alternative Gamma shape k over a grid and
# lay the six sweep values out as a 3x2 grid of MDR panels next to a 3x2 grid of
# FDR panels, for the appendix sensitivity study.
# ==============================================================================
...
# ------------------------------------------------------------------------------
# Monte Carlo engine (identical to exp2 in setting2.R; shape_alt is the swept arg)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Sweep the alternative Gamma shape k (shape_alt)
# ------------------------------------------------------------------------------
ini <- 500
...
# Six Gamma-shape values for the appendix sensitivity study. The range is kept
# MODERATE on purpose (verified by _diag_setting2.R): across it all four
# baselines FT/SCT/RTK/Adj-SAFFRON keep terminal MDR above alpha. At k<=2 Adj-
# SAFFRON drops below alpha and at k>=4 RTK does, so those extremes are avoided.
k_list <- c(2.4, 2.6, 2.8, 3.0, 3.2, 3.4)

fig_dir <- file.path(code_dir, "setting1,2,3-appendix")
results_file <- file.path(fig_dir, "Setting1_varied_k_results.rds")
reuse_cache <- TRUE
...
if (reuse_cache && file.exists(results_file)) {
  ...
  all_results <- lapply(k_list, function(k_v) {
    cat(sprintf("Setting 1 sweep: shape_alt (k) = %.2f ...\n", k_v))
    exp2(m = m, n = n, ini = ini, alpha = alpha, reps = reps, D = D,
         shape_alt = k_v, scale_alt = 1,
         pi_low = 0.03, pi_high = 0.35,
         drop_center = 0.50, drop_width = 0.10,
         pi_bounds = c(0.01, 0.99), topk_window = topk_window)
  })
  names(all_results) <- paste0("k_", k_list)
...
cat("\n=== Setting 1 sweep terminal metrics (t = max) ===\n")
...
mdr_panels <- lapply(seq_along(k_list), function(i)
  build_panel(all_results[[i]], bquote(k == .(k_list[i])), "mdr"))
fdr_panels <- lapply(seq_along(k_list), function(i)
  build_panel(all_results[[i]], bquote(k == .(k_list[i])), "fdr"))
...
ggsave(file.path(fig_dir, "Setting1_varied_k.pdf"),
       plot = final_layout, width = 18, height = 12)
ggsave(file.path(fig_dir, "Setting1_varied_k.png"),
