# ==============================================================================
# APPENDIX SCRIPT: Online MDR Control - Setting 2 sensitivity to the signal mean
#
# This is the APPENDIX companion of code-semi/setting1,2,3/setting2.R. It keeps
# the SAME methodology as the main Setting 2 script:
...
#     confidence bands.
# The only change is that we SWEEP the alternative mean gs_mean over a grid and
...
# ------------------------------------------------------------------------------
# Monte Carlo engine (identical to exp1 in setting1.R; gs_mean is the swept arg)
# ------------------------------------------------------------------------------
...
# Six alternative-mean values for the appendix sensitivity study. A SMALL-mu
# range is used deliberately: it keeps FT/RTK/Adj-SAFFRON terminal MDR above
# alpha AND keeps SCT's later-segment FDR above OMDRC.DD's (SCT over-discovers
# under the surge, so its MDR stays <= alpha while its FDR exceeds DD's). At
# large mu the signal is too easy and the baselines stop failing.
mu_list <- c(1.5, 1.8, 2.1, 2.4, 2.7, 3.0)

fig_dir <- file.path(code_dir, "setting1,2,3-appendix")
results_file <- file.path(fig_dir, "Setting2_varied_mu_results.rds")
reuse_cache <- TRUE
...
cat("\n=== Setting 2 sweep terminal metrics (t = max) ===\n")
...
ggsave(file.path(fig_dir, "Setting2_varied_mu.pdf"),
       plot = final_layout, width = 18, height = 12)
ggsave(file.path(fig_dir, "Setting2_varied_mu.png"),
