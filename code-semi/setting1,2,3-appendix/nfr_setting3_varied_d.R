...
#                                the dimension d, WITH the NFR
#                                (anti-free-riding) barrier on BOTH OMDRC arms
#                                -- credited ledger, D_NFR = 100.
#
# This is the NFR companion of setting3_varied_d.R in this appendix folder. The
...
#     comparable with setting3_varied_d.R;
...
#     confidence bands; we SWEEP the feature dimension d over the same grid as
#     setting3_varied_d.R (3x2 MDR panels next to 3x2 FDR panels). The grid is
#     capped at d = 40 because d >= 50 GAM fits are prohibitively slow.
#
# NOTE on the two different "d"s in this script: the SWEPT feature dimension is
# `d` / `d_list`, whereas the NFR barrier window is the SEPARATE constant
...
# ------------------------------------------------------------------------------
# Monte Carlo engine (setting3_varied_d.R engine; d is the swept arg;
...
# Six dimension values, SAME grid as setting3_varied_d.R. Capped at 40 because
# d >= 50 additive-GAM fits are prohibitively slow.
d_list <- c(10, 15, 20, 25, 30, 40)

fig_dir <- file.path(code_dir, "setting1,2,3-appendix")
results_file <- file.path(fig_dir, "NFR_Setting3_varied_d_results.rds")
reuse_cache <- identical(Sys.getenv("NFR_REUSE_CACHE"), "1")

cat(sprintf("Starting Setting 3 dimension-sweep + NFR credited barrier (D_NFR=%d)...\n",
            D_NFR))
...
# ------------------------------------------------------------------------------
# Plotting (same conventions as setting3_varied_d.R; one panel per swept value)
# ------------------------------------------------------------------------------
my_colors <- c(
...
cat("\n=== Setting 3 dimension sweep terminal metrics (t = max, NFR arms) ===\n")
...
ggsave(file.path(fig_dir, "NFR_Setting3_varied_d.pdf"),
       plot = final_layout, width = 18, height = 12)
ggsave(file.path(fig_dir, "NFR_Setting3_varied_d.png"),
       plot = final_layout, width = 18, height = 12, dpi = 300)
cat("Figures written to", file.path(fig_dir, "NFR_Setting3_varied_d.pdf/png"), "\n")
