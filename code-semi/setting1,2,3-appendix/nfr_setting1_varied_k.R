...
#                                the Gamma shape k, WITH the NFR
#                                (anti-free-riding) barrier on BOTH OMDRC arms
#                                -- credited ledger, d = 100.
#
# This is the NFR companion of setting1_varied_k.R in this appendix folder and
# mirrors code-semi/nofreeride-setting1,2,3/nfr_setting1_fig.R:
#   * skewed components F0 = Exp(1), F1 = Gamma(shape_alt = k, scale_alt = 1),
#     as specified in Appendix B.3 of the manuscript;
#   * smooth DECREASING prior-drop prevalence path pi_t (0.35 -> 0.03);
...
#     confidence bands; we SWEEP the alternative Gamma shape k over the same
#     grid as setting1_varied_k.R (3x2 MDR panels next to 3x2 FDR panels).
#
...
# ------------------------------------------------------------------------------
# Monte Carlo engine (setting1_varied_k.R engine; shape_alt is the swept arg;
...
# ------------------------------------------------------------------------------
# Sweep the alternative Gamma shape k (shape_alt), scale fixed at 1.
# ------------------------------------------------------------------------------
ini <- 500
...
topk_window <- ini
scale_fixed <- 1   # Appendix B.3 specifies Gamma(k, 1)

# Six Gamma-shape values, SAME grid as setting1_varied_k.R.
k_list <- c(2.4, 2.6, 2.8, 3.0, 3.2, 3.4)

fig_dir <- file.path(code_dir, "setting1,2,3-appendix")
results_file <- file.path(fig_dir, "NFR_Setting1_varied_k_results.rds")
reuse_cache <- identical(Sys.getenv("NFR_REUSE_CACHE"), "1")

cat(sprintf("Starting Setting 1 k-sweep + NFR credited barrier (d=%d)...\n", D_NFR))
if (reuse_cache && file.exists(results_file)) {
  ...
  all_results <- lapply(k_list, function(k_v) {
    ...
    exp2(m = m, n = n, ini = ini, alpha = alpha, reps = reps, D = D,
         shape_alt = k_v, scale_alt = scale_fixed,
         pi_low = 0.03, pi_high = 0.35,
         drop_center = 0.50, drop_width = 0.10,
...
# ------------------------------------------------------------------------------
# Plotting (same conventions as setting1_varied_k.R; one panel per swept value)
# ------------------------------------------------------------------------------
my_colors <- c(
...
cat("\n=== Setting 1 k-sweep terminal metrics (t = max, NFR arms) ===\n")
...
ggsave(file.path(fig_dir, "NFR_Setting1_varied_k.pdf"),
       plot = final_layout, width = 18, height = 12)
ggsave(file.path(fig_dir, "NFR_Setting1_varied_k.png"),
       plot = final_layout, width = 18, height = 12, dpi = 300)
cat("Figures written to", file.path(fig_dir, "NFR_Setting1_varied_k.pdf/png"), "\n")
