...
#                                the surge peak height pi_high, WITH the NFR
#                                (anti-free-riding) barrier on BOTH OMDRC arms
#                                -- credited ledger, d = 100.
#
# This is the NFR companion of setting2_varied_pi.R in this appendix folder. The
# DGP is kept VERBATIM from that script (Setting 2 was NOT re-tuned; only
# Setting 1 received updated parameters):
#   * Gaussian location shift F0 = N(0,1), F1 = N(gs_mean, 1), gs_mean = 2.4;
...
#     kept so the NFR panels stay directly comparable with setting2_varied_pi.R;
#   * FT uses the nominal miss budget alpha;
#   * the SAME six procedures, colours, shapes, labels and Monte-Carlo 95%
#     confidence bands; we SWEEP the surge peak height pi_high over the same
#     grid as setting2_varied_pi.R (3x2 MDR panels next to 3x2 FDR panels).
#
...
# ------------------------------------------------------------------------------
# Monte Carlo engine (setting2_varied_pi.R engine; pi_high is the swept arg;
...
# Six surge-peak values, SAME grid as setting2_varied_pi.R.
pi_high_list <- c(0.18, 0.22, 0.26, 0.30, 0.34, 0.38)

fig_dir <- file.path(code_dir, "setting1,2,3-appendix")
results_file <- file.path(fig_dir, "NFR_Setting2_varied_pi_results.rds")
reuse_cache <- identical(Sys.getenv("NFR_REUSE_CACHE"), "1")

cat(sprintf("Starting Setting 2 pi_high-sweep + NFR credited barrier (d=%d)...\n",
            D_NFR))
...
# ------------------------------------------------------------------------------
# Plotting (same conventions as setting2_varied_pi.R; one panel per swept value)
# ------------------------------------------------------------------------------
my_colors <- c(
...
cat("\n=== Setting 2 pi_high sweep terminal metrics (t = max, NFR arms) ===\n")
...
ggsave(file.path(fig_dir, "NFR_Setting2_varied_pi.pdf"),
       plot = final_layout, width = 18, height = 12)
ggsave(file.path(fig_dir, "NFR_Setting2_varied_pi.png"),
       plot = final_layout, width = 18, height = 12, dpi = 300)
cat("Figures written to", file.path(fig_dir, "NFR_Setting2_varied_pi.pdf/png"), "\n")
