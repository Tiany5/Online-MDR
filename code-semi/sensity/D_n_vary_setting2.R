# ==============================================================================
# SCRIPT: Sensitivity Analysis of OMDRC.DD -- APPENDIX version (Setting 2 DGP)
...
#
#   DGP: **Setting 2 of the main text**. One-dimensional Gaussian components,
#   F0 = N(0, 1), F1 = N(3, 1); the exact density ratio
#   r(x) = dnorm(x, 3, 1) / dnorm(x, 0, 1) gives the oracle Lmdr. The signal
#   proportion pi_t follows Setting 2's smooth prevalence SURGE
#   (0.05 -> 0.30 -> 0.05, center 0.60, width 0.15) over warm-up + stream;
#   the stream segment is anchored at k0_ref = 500 so it is IDENTICAL across
#   every K0 value. The non-monotone surge makes the trailing window lag pi_t
#   in BOTH directions, so the beta- and K0-sweeps expose the bias-variance
#   trade-off of the window construction, while the n-sweep shows the graded
#   convergence of DD to OR as the labeled reference grows.
#
#   This is the APPENDIX companion of sensity/D_n_vary.R (Setting 1, main
#   text); results/figures use the *_setting2 suffix so both coexist.
#
...
# Setting 2's smooth prevalence SURGE path (low -> high -> low): a Gaussian
# bump makes the trailing window lag pi_t on both the rise and the fall.
...
run_simulation <- function(m, ini, n, alpha, reps, D,
  ...
  mx <- max(m)
  if (pi_mode == "vary") {
    # Setting 2's prevalence surge over warm-up + stream. When k0_ref is given
    # the path is built over k0_ref + mx and the STREAM segment is anchored at
    # its last mx points, so pi_t over the evaluation stream is IDENTICAL for
    # every ini; the warm-up segment is the ini points immediately preceding
    # the stream (padded with pi_low when ini > k0_ref -- the surge starts
    # from its low baseline anyway).
    N_ref <- (if (is.null(k0_ref)) ini else k0_ref) + mx
    pi_path <- make_pi_surge(N_ref, pi_low = pi_low, pi_high = pi_high,
    ...
    if (is.null(k0_ref)) {
      ...
      pi_ini <- if (ini <= k0_ref) {
        ...
        c(rep(pi_low, ini - k0_ref), pi_path[seq_len(k0_ref)])
      }
    }
  } else {
  ...
  result <- foreach(r = 1:reps, .packages = c("mgcv")) %dopar% {
    ...
    z_ini <- rnorm(ini)
    z_ini[theta_ini == 1] <- rnorm(sum(theta_ini == 1), mean = gs_mean, sd = 1)

    # ---- Evaluation stream (1-D Gaussian components, time-varying pi) ----
    ...
    r_true <- dnorm(z_stream, mean = gs_mean, sd = 1) /
      dnorm(z_stream, mean = 0, sd = 1)
    lmdr_stream <- (pi_stream * r_true) / ((1 - pi_stream) + pi_stream * r_true)
    r_ini <- dnorm(z_ini, mean = gs_mean, sd = 1) /
      dnorm(z_ini, mean = 0, sd = 1)
...
# --- Setting 2 DGP parameters (1-D Gaussian components) ---
gs_mean_v <- 3        # F1 = N(gs_mean, 1) vs F0 = N(0, 1)

# --- Prevalence path: Setting 2's smooth prevalence surge ---
pi_mode_v      <- "vary"
pi_low_v       <- 0.05
pi_high_v      <- 0.30
surge_center_v <- 0.60
...
# ------------------------------------------------------------------------------
results_file <- file.path(code_dir, "sensity", "D_n_vary_setting2_results.rds")
reuse_cache  <- FALSE  # set FALSE to rerun everything from scratch
...
sim_common <- function(...) {
  run_sim_retry(...,
                ...
                surge_center = surge_center_v, surge_width = surge_width_v,
                pi_bounds = pi_bnds)
}
...
custom_theme <- theme_bw() +
  theme(
    ...
    # size 14 (not 16): the sweep row labels ("OMDRC.DD (K0=1000)") are wider
    # and would otherwise overflow the 10-inch device at ncol = 3.
...
ggsave(file.path(code_dir, "sensity", "Rplot_D_n_sensitivity_setting2.pdf"),
       final_plot, width = 10, height = 16)
ggsave(file.path(code_dir, "sensity", "Rplot_D_n_sensitivity_setting2.png"),
