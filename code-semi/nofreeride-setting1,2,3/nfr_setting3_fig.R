# ==============================================================================
# SCRIPT: Main-text Setting (c), including the local barrier in Algorithms 1
...
#   OMDRC.OR      -> OMDRC.OR+NFR   = OMDRC_OR_NFR(true 20-dim q, alpha, d)
...
exp3 <- function(m, n, ini, alpha, reps, D,
  ...
  result <- foreach(
    ...
    # features (no Isolation-Forest dimension reduction, no KDE). Only its
    # ESTIMATED score path is used; the decisions come from the barrier below.
...
fig_dir <- nfr_dir
results_file <- file.path(fig_dir, "NFR_Setting3_results.rds")
...
ggsave(file.path(fig_dir, "NFR_Setting3_fig.pdf"),
       plot = final_plot3, width = 10, height = 5)
ggsave(file.path(fig_dir, "NFR_Setting3_fig.png"),
       plot = final_plot3, width = 10, height = 5, dpi = 300)
cat("Figures written to",
    file.path(fig_dir, "NFR_Setting3_fig.pdf/png"), "\n")
