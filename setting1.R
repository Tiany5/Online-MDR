#' Simulation and Visualization for Online MDR Control (Setting 1)
#' 
#' This script reproduces Figure (a.1) and (a.2) using the Ratio of Expectations 
...
run_simulation_setting1 <- function(m, n, ini, alpha, reps, D) {
  ...
  results_list <- foreach(r = 1:reps, .packages = c("kedd", "onlineFDR")) %dopar% {
    ...
    N <- max(m) + ini
    theta <- rbinom(N, 1, 0.1)
    z <- ifelse(theta == 0, rnorm(N, 0, 1), rnorm(N, ifelse(rbinom(N, 1, 0.5) == 1, mu1, mu2), sd1))
  ...
  # Final Ratio calculation: E[Num] / E[Den]
  return(list(
...
# --- 4. Plotting Configuration (Your original styles) ---
...
# Save the entire working environment (including matrices, results, and plots)
save.image(file = "Setting1_workspace.RData")
