#' Sensitivity Analysis for Signal Strength (mu) in Setting 1
#' 
#' This script evaluates the impact of varying the signal strength parameter (mu)
#' on MDR and FDR control. Calculations use the Ratio of Expectations formula.
#' 
#' Author: [Anonymous]
#' Date: January 2025
...
# --- 2. Simulation Function for Varying mu ---
...
# --- 3. Plotting Configuration (Standard Styles) ---
...
# --- 4. Visualization Helper (FIXED ALIGNMENT) ---
...
plot_panel <- function(res, mu_val, type = "MDR") {
  ...
  # CRITICAL FIX: Use as.vector(t(mat)) to align rows with labels
  df <- data.frame(
